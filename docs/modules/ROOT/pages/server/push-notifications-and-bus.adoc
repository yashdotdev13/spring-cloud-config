...
[[ignored-monitor-paths]]
== Ignoring Paths

You can configure the Config Server to ignore specific paths when processing webhook notifications by setting `spring.cloud.config.server.monitor.ignored-paths` to a list of regular expressions.

Ignored paths are excluded before the Config Server determines which applications should be refreshed. This can be useful when a repository contains files that should not trigger configuration refreshes.
...
Each regular expression is matched against the complete webhook path. A path is ignored when it matches any configured expression.

By default, the Config Server processes at most 20 paths from a single notification. You can change this limit with `spring.cloud.config.server.monitor.max-paths`. If a notification contains more paths than the configured maximum, the remaining paths are not processed and a warning is logged.
